...
public class EditorComSelector : ScriptableWizard
{
	...
	static public void Draw<T> (string buttonName, T obj, OnSelectionCallback cb, params GUILayoutOption[] options) where T : MonoBehaviour
	{
		...
		if (o != null && Selection.activeObject != o.gameObject && GUILayout.Button("±à¼­", GUILayout.Width(40f)))
		{
	...
	static void Show<T> (OnSelectionCallback cb) where T : MonoBehaviour
	{
        ...
        EditorComSelector comp = ScriptableWizard.DisplayWizard<EditorComSelector>("Select " + type.ToString());
		comp.mType = type;
		comp.mCallback = cb;
