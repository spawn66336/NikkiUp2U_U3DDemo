...
public class UnityInternalIconCache 
{

    class CacheItem
    {
        public CacheItem(string p )
        {
            path = p;
        }
        ...
        public Texture Image
        {
            get
            {
                if( image == null )
                {
                    image = EditorGUIUtility.Load(path) as Texture2D;
                }
                return image;
            }
        }
    ...
    private UnityInternalIconCache()
    {
        _InitCache();
    }

    public Texture GetCacheIcon( string name )
    {
        ...
        CacheItem icon = null;
        iconCache.TryGetValue(name,out icon);
        return icon.Image;
    }
    ...
    private void _InitCache()
    {
        ...
        _BuildCacheItems(pathList);
    }

    private void _BuildCacheItems(List<string> pathList )
    {
        foreach( var path in pathList )
        {
            CacheItem newItem = new CacheItem(path);
            string fileName = Path.GetFileNameWithoutExtension(path); 
            iconCache.Add(fileName, newItem);
        }
    }


    private Dictionary<string, CacheItem> iconCache = new Dictionary<string, CacheItem>();
    ...
    private static UnityInternalIconCache s_instance = new UnityInternalIconCache();
